...
average_ed_los_flows<- function(start_date = as.POSIXct("2012-01-01 00:00:00", tz = "Europe/London"),
  ...
  dt_los <- spell_table %>%
    dplyr::select(spell_number, spell_start, initial_ed_end_datetime, spell_class_col, starts_with_ed,disposal_code, ed_admission,
                  source_referral_ae, directorate, hrg_ae_code, main_specialty_start) %>%
    dplyr::filter(spell_start > start_date | initial_ed_end_datetime < end_date) %>%
    dplyr::filter(starts_with_ed == TRUE)

  dt_flows <- dt_los %>%
    dplyr::mutate(flow_groups = make_flow_groups(dt_los))
