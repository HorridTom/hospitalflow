...
#'
#' @description
#' \code{plot_los_distribution_admission_method} shows the distribution of length of
#' stay for people in the inpatient department, stratified by the type of discharge.
#'
#' @param data Hospital episode data.
#' @inheritParams plot_admissions_discharges_day_of_week
#'
#' @return A plot (default) or a dataframe showing the distribution of length of
#' stay for hospitalized patients.
#'
#' @examples
#' \dontrun{
#' TBD
#' }
#' @export
plot_los_distribution_admission_method <- function(data,
                                                   startDate,
                                                   endDate,
                                                   returnPlot,
  ...
  start_date <- as.Date(startDate, tz = time_zone)
  end_date <- as.Date(endDate, tz = time_zone)
  ...
  breaks <- c(
    ...
    5.80, 5.16, 6.00,
    6.80, 6.16, 7.00,
    14.00, 21.00, 28.00, max(append(35, los), na.rm = TRUE)
  ) # , , na.rm = FALSE
  ...
  plot_elect_emerg <- ggplot2::ggplot(df_wrd_c, ggplot2::aes(x = losbinned, y = Count, group = admission_method_type, fill = admission_method_type)) + # alpha= ward.type - this gives a ligther version of the colours I choose   # changing the size of the bars for each categ change the parameter width, and in between the stacked categories the white colour is passed on the color command, with the width of the line 0.5
    ...
    ) + # changing the parameters in this box will give a different size of the subtitle
    ggplot2::theme(
