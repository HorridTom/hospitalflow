...
four_hrs_perf_flow_groups <- function(start_dt, end_dt, data, time_unit = "day", plot_chart, hospital_name = "Hospital_Name"){
  ...
  dt_select <- spelltable_sample %>%
    dplyr::filter(spell_start <= end_dt | spell_end >= start_dt) %>%
    dplyr::arrange(spell_start) %>%
    dplyr::mutate(flow_groups = dplyr::case_when(starts_with_ed == TRUE | directorate == NA  | ed_admission == FALSE ~ "Flow A",
                                                 starts_with_ed == TRUE | directorate == "Medical" | ed_admission == TRUE ~ "Flow 3",
                                                 starts_with_ed == TRUE | directorate == "Surgical" | ed_admission == TRUE ~ "Flow 4"))
