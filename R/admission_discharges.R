...
admission_discharges <- function(start_date, end_date, data, plot_chart, hospital_name = "Chelsea & Westminster"){
  ...
  end_date <- as.POSIXct( "2016-01-01 00:00:00", tz = "Europe/London")

  #subseting data set
  admissions_discharges <- dt_los  %>%
  ...
  dt_adm <- admissions_discharges  %>%
    dplyr::select(IDcol, Adm, EpisodeNumber, Los, Adm_period) %>%
  ...
  dt_adm_calc <- dplyr::full_join(dt_adm, df_adm_wkday, by = c("Adm", "Weekday", "Day"))
  ...
  dt_disch <- admissions_discharges  %>%
    dplyr::select(IDcol, Disch, EpisodeNumber, Los, Disch_period) %>%
  ...
  dt_disch_calc <- dplyr::full_join(dt_disch, df_disch_wkday, by = c("Disch", "Weekday", "Day"))
  ...
  non_emergency_adm <- admissions_discharges  %>%
    dplyr::select(IDcol, Adm, EpisodeNumber, Los, Adm_period, PatientType) %>%
  ...
  start_date_title <- format(start_date, format = "%d %B %Y")
  end_date_title <- format(end_date, format = "%d %B %Y")
