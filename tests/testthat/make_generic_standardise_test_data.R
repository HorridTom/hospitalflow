...
make_generic_standardise_test_correct_result <- function() {
  ed_test_result_str <- dplyr::tibble(
    pseudo_id = c("1", "2", "2", "3"),
  ...
  ip_test_result_str <- dplyr::tibble(
    pseudo_id = c("5", "6", "2", "4"),
  ...
  ed_test_result <- ed_test_result_str %>% dplyr::mutate(
  ...
  ip_test_result <- ip_test_result_str %>% dplyr::mutate(
    start_datetime = as.POSIXct(start_datetime, format = "%Y-%m-%d %H:%M:%S", tz = "Europe/London"),
...
make_generic_standardise_test_import_list <- function() {
  list(
    list(
      data_path = "testdata/generic_standardise/ed1.csv",
    ...
    list(
      data_path = "testdata/generic_standardise/ed2.csv",
      config_path = "testdata/generic_standardise/ed_config/",
      site = "B",
      facility = "ED",
      time_zone = "Europe/London"
    ...
    list(
      data_path = "testdata/generic_standardise/ip1.csv",
    ...
    list(
      data_path = "testdata/generic_standardise/ip2.csv",
      config_path = "testdata/generic_standardise/ip_config/",
      site = NA,
      facility = "IP",
      time_zone = "Europe/London"
