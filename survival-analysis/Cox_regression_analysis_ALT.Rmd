...
#ipsss <- lapply(dates, hospitalflow::get_inpatient_snapshot, )
predictions_data_2 <- tibble(t0 = dates) %>%
  dplyr::mutate(newdata = purrr::map(t0, hospitalflow::get_inpatient_snapshot,
                                     df = dfSurv2),
...
predictions_data_2 <- predictions_data_2 %>%
  dplyr::mutate(predicted = purrr::map(prob_here_48h, sum),
                actual = purrr::map(t0, hospitalflow::get_residual_occupancy,
...
model_output_CM2 <- hospitalflow::evaluate_predictions(predictions_data_2 %>% rename(dates = t0))
