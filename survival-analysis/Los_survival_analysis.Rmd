...
surv_data <- readRDS(file = "../../lgt-data/data-out/spell_table_lgt_20190815_944e22e6.rds")
...
dates <- seq(lubridate::ymd_hm('2018-09-01 12:00', tz = "Europe/London"), lubridate::ymd_hm('2018-11-30 12:00', tz = "Europe/London"), by = '1 day')
...
```{r eval-coxph2}
CM2 <- coxph(Surv(obsT, status) ~ adm_wd + gender + age_band_start, data = dfSurv2)
newdata2 <- ipss %>% select(obsT = stay_duration, adm_wd, gender, age_band_start) %>% mutate(status = TRUE)
...
model_output_CM2 <- hospitalflow::run_S_func_model(df = dfSurv2,
                                                   coxmodel = "Surv(obsT, status) ~ adm_wd + gender + age_band_start",
                                                  date_seq = dates)
...
~0% non-emergency medical

AAU - more consistency 7 day
Downstream - much less so - could assume no weekend discharges?

Before tacking this, we need a general framework within which these models can be deployed and evaluated.
