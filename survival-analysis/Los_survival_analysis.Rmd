...
surv_data <- surv_data %>% filter(substr(as.character(ADMISSIONMETHOD), 1, 1) == "2")
...
eventT <- surv_data %>% filter(!is.na(los) & los >= 0 & end_datetime < lubridate::ymd_hm('2018-11-01 00:00')) %>% pull(los)
adm_wd <- surv_data %>% filter(!is.na(los) & los >= 0 & end_datetime < lubridate::ymd_hm('2018-11-01 00:00')) %>% pull(adm_wd)
...
dfSurv2 <- data.frame(eventT, obsT, status, adm_wd)
```
...
dates <- seq(lubridate::ymd_hm('2018-11-01 12:00', tz = "Europe/London"), lubridate::ymd_hm('2019-01-31 12:00', tz = "Europe/London"), by = '1 day')
