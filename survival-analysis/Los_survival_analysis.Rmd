...
```{r eval-coxph2}
CM2 <- coxph(Surv(obsT, status) ~ adm_wd + adm_calmonth + gender + age_band_start, data = dfSurv2)
newdata2 <- ipss %>% select(obsT = stay_duration, adm_wd, adm_calmonth, gender, age_band_start) %>% mutate(status = TRUE)
...
model_output_CM2 <- hospitalflow::run_S_func_model(df = dfSurv2,
                                                   coxmodel = "Surv(obsT, status) ~ adm_wd + adm_calmonth + gender + age_band_start",
                                                  date_seq = dates)
