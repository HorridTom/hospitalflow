...
predict_residual_occupancy <- function(df, t, S_function, delta_t) {
  ...
  ipss1 <- ipss1 %>% mutate(prob_here_48h = hospitalflow::get_discharge_probability(stay_duration, S_function, delta_t))
...
system.time(predictions <- predictions %>% dplyr::mutate(predicted = pred_res_occ_v(df = surv_data, dates,
                                                                                    S_function = S_function,
                                                                                    delta_t = 48)) %>%
  dplyr::mutate(actual = get_res_occ_v(df = surv_data, dates, delta_t = 48)) %>%
